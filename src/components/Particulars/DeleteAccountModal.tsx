...
const DeleteAccountModal = ({deleteAccountModal, setDeleteAccountModal, deleteAccount, setDeleteAccount, getInfosParticular}: modalDeleteAccountProps) => {
    ...
    return (
      ...
      {deleteAccount && (
        <div className={`w-full h-full flex flex-col justify-center items-center`}>
          ...
          <button
            onClick={() => {setDeleteAccount && setDeleteAccount(false); logOut().then( () =>{
                return navigateFromDeleteAccount("/")
                })}}
            className={`${button} flex justify-center items-center mt-6 w-[45%]`}>
            <Link to="/particular/vehicules" className='w-full h-full'>Retour
            </Link>
          </button>
        </div> 
      </div>      
