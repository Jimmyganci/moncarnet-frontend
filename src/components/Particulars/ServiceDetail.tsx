...
import { glassMorphism, glassMorphismWhiteShadow, button } from '../../variableTailwind';
    ...
    return (
        <div className='h-full w-screen flex justify-center items-center'>
            <div className={`${glassMorphism} w-11/12 h-5/6 my-4 rounded-lg py-4 px-2`}>
                <h3>
                    <span className='border-r-2 border-background pr-2'>{infosVehicule.length && (infosVehicule[0].brand + " " + infosVehicule[0].model)}</span>
                    <span className='pl-2'>{infosVehicule.length && infosVehicule[0].immat}</span>
                </h3>
                <form className={`flex flex-col w-11/12 h-fit mx-auto rounded-lg p-2 items-center justify-center`}>
                    <label className="flex flex-col w-full">
                        <span className="text-lg font-semibold">Date</span>
                        <div
                        className={`${glassMorphismWhiteShadow} h-fit py-1 my-2`}
                        >{infosService.date && (`${infosService.date.slice(8, 10)}/${infosService.date.slice(5, 7)}/${infosService.date.slice(0, 4)}`)}
                        </div>
                    </label>
                    <label className="flex flex-col w-full">
                    ...
                    <Link className='w-fit h-fit mt-2' to={`/particular/vehicules/${vehiculeImmatToUpdate}/serviceBook`}><button className={`w-fit h-fit p-2 px-4 ${button}`}>Retour au carnet</button></Link>
            </div>
        </div>
    );
};
