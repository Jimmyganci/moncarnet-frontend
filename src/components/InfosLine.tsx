    ...
    return (
            <div className={`h-10 w-full rounded-xl my-1 flex justify-center items-center`}>
               <label className='w-full h-full max-w-2xl mx-2 flex justify-center items-center'>
                  <p onClick={() => {(changeMode && !lineChangeMode)? setLineChangeMode(!lineChangeMode) : ""}}
 className={`${lineChangeMode && changeMode? "hidden w-full" : ""} ${changeMode? "cursor-pointer opacity-50" : ""} p-2 mb-4 text-center truncate overflow-hidden border rounded-md h-full w-5/6 bg-slate-50/75 hover:h-fit hover:overflow-visible hover:whitespace-pre-wrap`}>
                 {modif || lineName}</p>
