...
function ModalAppointment() {
  ...
  const { prosLoggedIn, showModalServiceBook, setShowModalServiceBook, appointmentId } = useContext(ProsContext);
...
// Create ServiceBook
  async function createServiceBook() {
    if (dayUpdate && serviceUpdate && observationUpdate && kilometrage && prosLoggedIn.id_user ) {
      try {
        const res =
        prosLoggedIn &&
