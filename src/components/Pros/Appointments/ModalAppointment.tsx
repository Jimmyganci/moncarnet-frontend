...
import { glassMorphism, h3, button, deleteButton, input } from '../../../variableTailwind';
...
const ModalAppointment = ({ date, user, comment, id_appointment }: InfosRdv) => {
  ...
  return (
    <main className='fixed inset-0 z-10 flex justify-center w-screen h-screen align-middle' onClick={() => handleParentsClick() }> 
      <section className={`m-16 p-8 rounded-lg w-4/6 ${glassMorphism}`} onClick={(e) => handleChildClick(e)}>
      {
        changeMode ?
        <div className='flex flex-col items-center justify-around h-full'>
          ...
          />
           {
            message === 200 && <p className='text-green-700'>Votre rendez-vous a bien été modifié</p>
          }
          {
            message === 500 && <p className='text-red-500'>Impossible de modifier ce rendez-vous</p>
          }
        ...
        <div className='flex flex-col items-center justify-around h-full'>
          ...
          <div className='flex mb-4'>
            <button className={`w-32 h-12 ${button}`} onClick={() => !changeMode ? setChangeMode(!changeMode) : updateAppointment()}>{changeMode ? "Valider" : "Modifier"}</button>
